...
public class AINavAgent : AIAgent
{
	...
	void Update()
	{
		...
		{
			AINavNode destinatioNode = AINavNode.GetRandomAINavNode();
			path.destination = destinatioNode.transform.position;
		}
	}
	...
	{
		float squaredRangeA = (a.transform.position - transform.position).sqrMagnitude; //avoids square root operation
